...
/**
    ...
    recorimos e rotacionamos.
 */
void inserirCaso1 (structNo* avo, structNo* noPai, structNo* chaveNo);

/**
 * - Caso 2: Caso contrário, recorimos e repetimos o ciclo até obtermos
    o equilíbrio.
 */
...
structNo *sibling(structNo* no) {
    // sibling null if no pai
    if (no->pai == NULL)
        return NULL;
...
int hasRedChild(structNo) {
    return (left != NULL and left->color == RED) or
            (right != NULL and right->color == RED);
}


void fixDoublepreto(structNo *x) {
   // if (x == root)
        // Reached root
       // return;
    if(x->pai == NULL){
    ...
    structNo *sibling = x->sibling(x), *pai = x->pai;
    if (sibling == NULL) {
        // No sibiling, double preto pushed up
        fixDoublepreto(pai);
    } else {
        if (sibling->cor == vermelho) {
            ...
            if (sibling->hasvermelhoChild()) {
                // at least 1 vermelho children
                if (sibling->left != NULL and sibling->left->cor == vermelho) {
                    if (sibling->isOnLeft()) {
                        // left left
                        sibling->left->cor = sibling->cor;
                        sibling->cor = pai->cor;
                        rightRotate(pai);
                    } else {
                        // right left
                        sibling->left->cor = pai->cor;
                        rightRotate(sibling);
                        leftRotate(pai);
                    }
                } else {
                    if (sibling->isOnLeft()) {
                        // left right
                        sibling->right->cor = pai->cor;
                        leftRotate(sibling);
                        rightRotate(pai);
                    } else {
                        // right right
                        sibling->right->cor = sibling->cor;
                        sibling->cor = pai->cor;
                        leftRotate(pai);
                    }
                }
...
void deletestructNo(structNo *v) {
    structNo* raiz;
    structNo *u = BSTreplace(v);

    // True when u and v are both preto
    int uvpreto = ((u == NULL || u->cor == preto) && (v->cor == preto));
    structNo *pai = v->pai;
    ...
    if (u == NULL) {
      ...
      //  } else {
            if (uvpreto) {
                // u and v both preto
                // v is leaf, fix double preto at v
                fixDoublepreto(v);
            } else {
                // u or v is vermelho
                if (v->sibling() != NULL)
                    // sibling is not null, make it vermelho"
    ...
    if (v->left == NULL or v->right == NULL) {
        ...
        if (v == root) {
            ...
            u->pai = pai;
            if (uvpreto) {
                // u and v both preto, fix double preto at u
                fixDoublepreto(u);
            } else {
                // u or v vermelho, cor u preto
                u->cor = preto;
            }
        }
        return;
